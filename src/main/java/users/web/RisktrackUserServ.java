...
@WebServlet(urlPatterns = {"/RiskTrackUser"})
public class RisktrackUserServ extends TopServlet{

    static Logger logger = LogManager.getLogger(RisktrackUserServ.class);
    static String appDbInfo = null;
    static String[] roleNames = {"Edit","Edit & Delte","Admin"};
    static String[] roles = {"Edit","Admin:Edit","Admin"};
	...
	String message = "";
	appDbInfo = getAppDbInfo("risktrack");
	RisktrackUser user = new RisktrackUser();
	...
	String back = tl.findRisktrackUsers();
	if(back.isEmpty()){
	    users = tl.getUsers();
	}
	...
	if(id.isEmpty()){	
	    ...
	    out.println("<a href=\""+url+"RiskTrackUser\">New User</a>");
	}
