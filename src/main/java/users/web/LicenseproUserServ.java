...
@WebServlet(urlPatterns = {"/LicenseproUser"})
public class LicenseproUserServ extends TopServlet{

    static Logger logger = LogManager.getLogger(LicenseproUserServ.class);
    static String appDbInfo = null;
    static String[] roleNames = {"View", "Edit","Edit & Delte","Admin"};
	...
	String message = "";
	appDbInfo = getAppDbInfo("licensepro");
	LicenseproUser user = new LicenseproUser();
	...
	out.println("<font size=+1 color=green> "+
		    "Licensepro Application </font><br>"); 
	out.println("<br />");
	if(!message.equals("")){
	    out.println(message);
	    out.println("<br />");
	...
	String back = tl.findLicenseproUsers();
	if(back.isEmpty()){
	    users = tl.getUsers();
	}
